...
function MediaTemplate(props) {
          ...
          <div className="card border-0">
            <div class="d-flex align-items-center ">
              ...
              <div class="flex-grow-1 ms-1 mt-3">
                ...
                <PostDate postdate={comment.commented_at}></PostDate>
                {/* <p>{comment.comment}</p> */}
              </div>
            </div>
          </div>
  ...
  async function checkLike(like) {
    if (like.target.checked) {
      const docRef = await updateDoc(doc(db, `forumposts/${like.target.id}`), {
    ...
    if (props.likes.includes(currentUser.uid)) {
      ...
      return (
        <>
          ...
          <h6 class="px-3 mt-2">{props.likes.length} person likes this.</h6>
        </>
      );
    } else {
      // setChecked(false)
    ...
    <Fragment key={post.id}>
      <div class="border border-dark-subtle pb-3 rounded-3 mb-3" id="postsDiv">
        ...
        <div class="px-3 border-top">
          ...
          <GetComments allcomments={post.data().comments}></GetComments>
  ...
  return (
    <>
      <h4 class="mt-5 text-center mb-4">
